...
type ReservationRow struct {
	...
	ReservationId ReservationId       `bun:"reservation_id,pk"`
...
// BunReservationRepository is the Postgres-backed ReservationRepository
// implementation. SaveReservation stages the write inside the supplied
// TransactionalContext so the INSERT runs against the live tx handle
// resolved via tx.TxFromContext. Reads bypass the tx substrate.
type BunReservationRepository struct {
	db *bun.DB
}

// Compile-time assertion that BunReservationRepository satisfies
// ReservationRepository. If a method signature drifts, the assertion fails
// before any test runs.
var _ ReservationRepository = (*BunReservationRepository)(nil)

// NewBunReservationRepository constructs a BunReservationRepository bound
// to db. The caller owns the *bun.DB lifecycle.
func NewBunReservationRepository(db *bun.DB) *BunReservationRepository {
	return &BunReservationRepository{db: db}
}

// SaveReservation stages an upsert keyed by reservation_id. The stage
// closure runs immediately inside the bun tx callback.
func (r *BunReservationRepository) SaveReservation(_ context.Context, reservation ReservationDto, txc tx.TransactionalContext) error {
...
// (nil, nil) on miss.
func (r *BunReservationRepository) FindReservationById(ctx context.Context, reservationId ReservationId) (*ReservationDto, error) {
...
func (r *BunReservationRepository) ListReservationsForBook(ctx context.Context, bookId catalog.BookId) ([]ReservationDto, error) {
...
func (r *BunReservationRepository) ListReservationsForMember(ctx context.Context, memberId membership.MemberId) ([]ReservationDto, error) {
	var rows []ReservationRow
	err := r.db.NewSelect().
		Model(&rows).
...
func (r *BunReservationRepository) PendingReservationCountForBook(ctx context.Context, bookId catalog.BookId) (int, error) {
...
func toReservationDto(row ReservationRow) ReservationDto {
...
func toReservationDtos(rows []ReservationRow) []ReservationDto {
	reservations := make([]ReservationDto, 0, len(rows))
